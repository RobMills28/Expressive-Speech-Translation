    ...
    def initialize(self):
        ...
        try:
            ...
            if self.use_voice_cloning:
                ...
                try:
                    self.tone_converter = ToneColorConverter(self.config_path, device=self.device)
                    self.tone_converter.load_ckpt(self.checkpoint_path)
                    self.source_se = torch.load(self.speaker_path, map_location=self.device)
            ...
            logger.info("Cascaded backend initialized successfully")
