...
def load_all_model(
    ...
    # Get the absolute path to the project root (/app/MuseTalk) by going up TWO levels
    # from this file's directory (/app/MuseTalk/musetalk/utils)
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    
    # Build absolute paths if none are provided
    ...
    vae_model_path = os.path.join(project_root, "models", vae_type)
...
def get_mouth_region(frames, image_pred, pixel_values_face_mask):
    mouth_real_list, mouth_generated_list = [], []
...
def process_audio_features(cfg, batch, wav2vec, bsz, num_frames, weight_dtype):
    with torch.no_grad():
        ...
        audio_feats = torch.cat([torch.zeros_like(audio_feats[:, :2*cfg.data.audio_padding_length_left]), audio_feats, torch.zeros_like(audio_feats[:, :2*cfg.data.audio_padding_length_right])], 1)
        audio_prompts = []
        for bb in range(bsz):
            audio_feats_list = []
...
def delete_additional_ckpt(base_path, num_keep):
    ...
    if len(dirs) <= num_keep:
        return
    del_dirs = sorted(dirs, key=lambda x: int(x.split("-")[-1]))[:len(dirs) - num_keep]
    for d in del_dirs:
...
def process_and_save_images(batch, image_pred, image_pred_infer, save_dir, global_step, accelerator, num_images_to_keep=10, syncnet_score=1):
    ...
    concat = torch.cat([masked_pixel_values * 0.5 + 0.5, pixel_values_ref_img * 0.5 + 0.5, image_pred * 0.5 + 0.5, pixel_values * 0.5 + 0.5, image_pred_infer * 0.5 + 0.5], dim=2)
