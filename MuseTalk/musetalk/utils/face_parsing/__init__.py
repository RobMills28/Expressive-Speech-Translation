...
class FaceParsing():
    ...
    def model_init(self, 
        ...
        # Get the absolute path to the project root (/app/MuseTalk) by going up three directories
        # from this file's location (/app/MuseTalk/musetalk/utils/face_parsing)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
        
        # Build absolute paths if none are provided, making the paths robust
        if resnet_path is None:
            resnet_path = os.path.join(project_root, 'models', 'face-parse-bisent', 'resnet18-5c106cde.pth')
        if model_pth is None:
            model_pth = os.path.join(project_root, 'models', 'face-parse-bisent', '79999_iter.pth')
