...
# Build absolute paths to the config and checkpoint files using the project root.
...
fa = FaceAlignment(LandmarksType._2D, flip_input=False, device=face_alignment_device)
...
def get_bbox_range(img_list,upperbondrange =0):
    ...
    average_range_minus = []
    average_range_plus = []
    for fb in tqdm(batches, desc="Getting bbox range"):
...
def get_landmark_and_bbox(img_list,upperbondrange =0):
    ...
    average_range_minus = []
    average_range_plus = []
    for fb in tqdm(batches, desc="Getting landmarks and bboxes"):
        results = inference_topdown(model, np.asarray(fb)[0])
        results = merge_data_samples(results)
        keypoints = results.pred_instances.keypoints
        face_land_mark= keypoints[0][23:91]
